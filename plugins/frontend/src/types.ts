...
/** All usage metrics the CRA UI can request (order is stable for tests and labels). */
...
/** Raw rows for mock dataset only (converted to `TrendPoint` in the mock data source). */
export type FinOpsCraMockTrendRow = {
  period: string;
  scope_slug: string;
  scope_name: string | null;
  provider_type: ProviderType;
  cost_value: number;
  /** Synthetic hours used to derive per-metric usage in the mock data source. */
...
export type FinOpsCraDataset = {
  scopes: ScopeItem[];
  teams: TeamItem[];
  trends: FinOpsCraMockTrendRow[];
};
